...
def get_cost(field):
    ...
    for index in range(len(field)-1):
        sum += get_dist(field[index],field[index+1])
    return sum
...
#получение первого
adams = [get_random_field(mode = 1) for x in range(number_of_islands)]
populations = []

for pop in range(number_of_islands):
    populations.append([])
    for subject in range(population_number): 
		#начальная популяция
